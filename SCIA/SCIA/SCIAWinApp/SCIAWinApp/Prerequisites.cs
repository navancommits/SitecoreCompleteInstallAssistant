...
namespace SCIA
{
    public partial class Prerequisites : Form
    {
        ...
        void WriteMainFile(string path)
        {
            ...
            file.WriteLine("$preference = $ProgressPreference");
            file.WriteLine("$ProgressPreference = \"SilentlyContinue\"");
            file.WriteLine("Expand-Archive -Force -LiteralPath " + prereqs.Where(p => p.PrerequisiteKey == commerceZipKey).ToList().FirstOrDefault().PrerequisiteName + ".zip -DestinationPath .\\" + prereqs.Where(p => p.PrerequisiteKey == commerceZipKey).ToList().FirstOrDefault().PrerequisiteName);
