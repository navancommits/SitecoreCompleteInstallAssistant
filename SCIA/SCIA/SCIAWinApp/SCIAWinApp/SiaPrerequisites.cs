...
namespace SCIA
{
    public partial class SiaPrerequisites : Form
    {
        ...
        private void CheckPrerequisites()
        {
            ...
            {
                SetStatusMessage("Missing Setup Exe - Sitecore 10.0.0 rev. 004346 (Setup XP0 Developer Workstation rev. 1.2.0-r64)\\setup.exe ", Color.Red);
                return;
            }
        ...
        void WriteMainFile(string path)
        {
            ...
            file.WriteLine("$preference = $ProgressPreference");
            file.WriteLine("$ProgressPreference = \"SilentlyContinue\"");
            file.WriteLine(".\\DownloadandSetupSiaPrereqs.ps1 -InstallSourcePath $InstallSourcePath -SitecoreUsername \"" + Login.username + "\" -SitecorePassword \"" + Login.password + "\"");
            file.WriteLine("Expand-Archive -Force -LiteralPath \"Sitecore 10.0.0 rev. 004346 (Setup XP0 Developer Workstation rev. 1.2.0-r64).zip\" -DestinationPath \"Sitecore 10.0.0 rev. 004346 (Setup XP0 Developer Workstation rev. 1.2.0-r64)\"");
            file.WriteLine(
                "Start-Process -FilePath \".\\Sitecore 10.0.0 rev. 004346 (Setup XP0 Developer Workstation rev. 1.2.0-r64)\\setup.exe\"");
            file.WriteLine();
            file.WriteLine(
        ...
        {
            //SiaPrerequisitesInstaller siaPrerequisitesInstaller = new SiaPrerequisitesInstaller();
            //siaPrerequisitesInstaller.ShowDialog();
            if (!Login.Success)
            {
            ...
            else
            {
                ...
                CommonFunctions.LaunchPSScript(".\\DownloadandExpandSiaZip.ps1 -InstallSourcePath \".\" -SitecoreUsername \"" + Login.username + "\" -SitecorePassword \"" + Login.password + "\"");
            }
        }
    }
}
